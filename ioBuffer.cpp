...
unsigned char inBuffer::get_8()
{
    if (out_pos + 1 > m_size)
    {
        throw std::runtime_error("inBuffer::get_8: noenough ");
    }
...
void inBuffer::unpack(uint8_t* buf, size_t sz)
{
    ...
    {
        throw std::runtime_error("unpack error not enough");
    }
...
void inBuffer::unpack(std::string& s, int64_t size)
{
    ...
    if (!success) throw std::runtime_error("inBuffer unpack:  buffer size to small");
    XPASS;
}
...
REF_getter<refbuffer> outBuffer::asString() const
{
    ...
    buffer->size=cur_pos;
    buffer->buffer=(uint8_t*)realloc(buffer->buffer,buffer->size+0x20);
    if(!buffer->buffer)
        throw std::runtime_error("alloc error");
    buffer->capacity=buffer->size+0x20;
    return buffer;
}
